...
class EMB(nn.Module):
    def __init__(self, params):
        ...
        if self.dim == 3: 
            ...
            self.emb_pos = nn.Parameter(torch.randn((self.img_hwd//self.patch_size)**3, self.emb_size)) # hwd/p^3, embedding size
        else: # 2D
            ...
            self.emb_pos = nn.Parameter(torch.randn((self.img_hwd//self.patch_size)**2, self.emb_size)) # hwd/p^2, embedding size
    ...
    def forward(self, x):
        ...
        if self.dim == 3: x = rearrange(x, 'b e (h) (w) (d) -> b (h w d) e')  # x = x.reshape(b, -1, self.emb_size).transpose(1, 2)
        else: x= rearrange(x, 'b e (h) (w) -> b (h w) e')
