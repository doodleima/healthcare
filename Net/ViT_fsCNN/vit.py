...
class EMB(nn.Module):
    def __init__(self, params):
        ...
        self.patch_size = int(params['patch_size'] * 2) #3
        ...
        init.kaiming_normal_(self.emb_pos, nonlinearity='relu') # replaced Weight Initialization code suggested by Kaiming He's

        # self.cls_token = nn.Parameter(torch.randn(1, 1, self.emb_size)) ### for classification - not applied yet
        # init.xavier_normal_(self.emb_pos, gain=init.calculate_gain('relu')) # Xavier-Glorot Initialization
...
class MLP(nn.Module):
    def __init__(self, params, num_layers=4):
        super(MLP, self).__init__()

        self.expansion = num_layers        
        self.emb_size = int(params['emb_size'])
        self.dropout_rate = float(params['drop_rate'])
...
class ViT(nn.Module):
    def __init__(self, params):
        ...
        if self.dim == 3:
            ...
            self.deconv_layer1 = nn.ConvTranspose2d(in_channels=self.emb_size, out_channels=self.input_c, kernel_size=self.patch, stride=self.patch)
            self.deconv_layer2 = nn.ConvTranspose2d(in_channels=self.emb_size, out_channels=self.input_c, kernel_size=self.patch1, stride=self.patch1)
            self.deconv_layer3 = nn.ConvTranspose2d(in_channels=self.emb_size, out_channels=self.input_c, kernel_size=self.patch2, stride=self.patch2)
            self.deconv_layer4 = nn.ConvTranspose2d(in_channels=self.emb_size, out_channels=self.input_c, kernel_size=self.patch3, stride=self.patch3)
